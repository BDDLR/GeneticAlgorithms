...
int main(int argc, char const *argv[])
{
	...
	if(argc != 2){
		cout << "Correct way: ./main <NUM_GENERATIONS>" << endl;
		exit(0);
	}
	...
	simple_ranking(initial_individuals);

	for (int i = 0; i < TOTAL_INDIVIDUALS; i++) {
    	total += get_expected_value(initial_individuals[i]);
    }			
	...
	for (int k = 0; k < NUM_GENERATIONS; k++)
	{
		...
		for (int i = 0; i < TOTAL_INDIVIDUALS; i++)
		{
			logGenerations << initial_individuals[i] << " -- " << roulette_selection[i] << " -- " 
...
void drawHistogram(int minGenerationValues[], int maxGenerationValues[], int generationValuesAverage[], int num_gen){
	...
	gfx_open(xsize,ysize,"Hierarchy-Roulette Selection Results");
